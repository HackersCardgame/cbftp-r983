...
//s->r002
    ...
    commandBNC(remainder);
  }
//s<-r002
//s->r005
  else if (command == "setdir") {
    commandSetDir(remainder);
  }
  else if (command == "setaffils") {
    commandSetAffils(remainder);
  }
//s<-r005
  else {
...
//s->r002
void RemoteCommandHandler::commandBNC(const std::vector<std::string> & message) {
  if (message.size() < 2) {
    global->getEventLog()->log("RemoteCommandHandler", "Bad remote bnc command format: " + util::join(message));
    return;
  }

  std::string sitestring = message[0];
  std::list<std::string> bncstring = util::trim(util::split(message[1], ","));
  std::string bnclist = util::join(bncstring);

  std::shared_ptr<Site> site = global->getSiteManager()->getSite(sitestring);

  if (!site) {
    global->getEventLog()->log("RemoteCommandHandler", "Site not found: " + sitestring);
  }
  else {
    global->getEventLog()->log("RemoteCommandHandler", "Updating " + sitestring + " BNC to: " + bnclist);
    site->setAddresses(bnclist);
  }
}
//s<-r002

//s->r005
void RemoteCommandHandler::commandSetDir(const std::vector<std::string> & message) {
  if (message.size() < 2) {
    global->getEventLog()->log("RemoteCommandHandler", "Bad remote setdir command format: " + util::join(message));
    return;
  }

  std::string sitename = message[0];
  std::list<std::string> dirstring = util::trim(util::split(message[1], ";"));

  std::shared_ptr<Site> site = global->getSiteManager()->getSite(sitename);

  if (!site) {
    global->getEventLog()->log("RemoteCommandHandler", "Site not found: " + sitename);
    const std::string & newSite = sitename;
    std::shared_ptr<Site> snew(new Site(newSite));
    global->getSiteManager()->addSite(snew);
    for (std::list<std::string>::const_iterator it = dirstring.begin(); it != dirstring.end(); ++it) {

      size_t splitpos = it->find(",");
      std::string section = *it;
      std::string path = "/";
      if (splitpos != std::string::npos) {
        section = it->substr(0, splitpos);
        path = it->substr(splitpos + 1);
      }

      global->getEventLog()->log("RemoteCommandHandler", "Updating " + newSite + " section " + section + " to: " + path);
      snew->addSection(section, path);
    }
  }
  else {
    site->clearSections();
    for (std::list<std::string>::const_iterator it = dirstring.begin(); it != dirstring.end(); ++it) {
      ...
      global->getEventLog()->log("RemoteCommandHandler", "Updating " + sitename + " section " + section + " to: " + path);
      site->addSection(section, path);
    }
  }
}

void RemoteCommandHandler::commandSetAffils(const std::vector<std::string> & message) {
  if (message.size() < 2) {
    global->getEventLog()->log("RemoteCommandHandler", "Bad remote setaffil command format: " + util::join(message));
    return;
  }

  std::string sitename = message[0];
  std::list<std::string> affils = util::trim(util::split(message[1], ","));
  std::string affilstring = util::join(affils);
  ...
  if (!site) {
    ...
    global->getEventLog()->log("RemoteCommandHandler", "Updating " + newSite + " affils to: " + affilstring);
    snew->addAffil(affilstring);
  }
  else {
    global->getEventLog()->log("RemoteCommandHandler", "Updating " + sitename + " affils to: " + affilstring);
    site->clearAffils();
    site->addAffil(affilstring);
  }
}
//s<-r005
